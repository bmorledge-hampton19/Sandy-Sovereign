...
public class EventViewer extends Menu implements ActionListener{
	...
	public void paintComponent(Graphics g) {
		...
		displayText(g, 600, 50, String.format("Turn: %d", manager.getCurrentTurn()), 20);
		...
		if (currentEvent == null) {
			...
			// Tell the player about this turn's income. (If not the first turn.)
			if (manager.getCurrentTurn() > 1) {
				displayText(g, 250, 330, "Turn income has been received.", 20);
				displayText(g, 250, 350, incomeInfo, 20);
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// This button confirms the results of the event.
			...
			// If the sandCastleBuilderEvent was not activated, get the turn's income.
			incomeInfo = manager.getStockpile().applyIncome();
